...
pub fn sys_get_time(_ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    *translated_refmut(token, _ts) = tv;
    0
}
...
pub fn sys_mmap(_start: usize, _len: usize, _port: usize) -> isize {
    ...
    if _len == 0 || _len % PAGE_SIZE != 0 {
        return -1;
    }
    let prot = _port & 0x7;
    if prot == 0 || (_port & !0x7) != 0 {
        return -1;
    }
    ...
    if inner.memory_set.mmap_area(start, end, perm) {
        0
    } else {
        -1
    }
...
pub fn sys_munmap(_start: usize, _len: usize) -> isize {
    ...
    let start = VirtAddr::from(_start);
    let end = VirtAddr::from(_start + _len);
    if inner.memory_set.munmap_area(start, end) {
        0
    } else {
        -1
    }
